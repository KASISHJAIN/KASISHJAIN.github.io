...
const projects = {
  vision: {
    ...
    problem: "Existing gesture-based interfaces don't account for what happens when the sensor fails or gives bad data. I'm building a system that can control hardware safely even under unreliable inputs — so I designed the failure modes before writing a single line of control logic.",
  ...
  rocketry: {
    ...
    built: "Onboard avionics using ESP32 microcontrollers with BMP280 pressure sensors for altitude tracking and SD card logging for post-flight telemetry analysis. Conducted system validation and hardware debugging during assembly and pre-flight testing to ensure reliable sensor communication under significant physical stress.",
  ...
  solar: {
    ...
    problem: "A community in rural Peru needed reliable clean water. Our EPICS team redesigned a containerized purification system — downsizing from a 40ft to 20ft container while keeping it fully functional, safe, and locally maintainable. Awarded EPICS Special Commendation.",
    built: "Co-designed a 10.3 kW bifacial solar array and 120 kWh LiFePO\u2084 battery system, performing detailed mathematical analysis — irradiance, depth-of-discharge, thermal loads — to optimize power storage. Rapidly mastered Autodesk Revit to create a full Construction Sheet Set including circuit schematics and wiring diagrams. Sourced affordable components from Peruvian vendors for long-term local maintainability.",
  ...
  jiramate: {
    ...
    built: "A full-stack web app using React, Node.js, and Firebase. Integrated Nemotron via OpenRouter API to automate user story and acceptance criteria generation. Added backend validation and prompt engineering to make LLM output reliable and structured enough to actually submit to Jira.",
  ...
  braille: {
    ...
    built: "Collaborating on a multidisciplinary SWE team to design a wearable system that detects finger-movement Braille patterns and translates them in real-time into text and audio output. Working through active hardware iteration and ergonomic testing.",
